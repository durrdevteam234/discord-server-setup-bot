    ...
    try {
      ...
      if (message.content && message.content.startsWith(prefix)) {
        ...
        if (commandName === 'setup') {
          ...
          const guild = message.guild; if (!guild) return;
          if (!message.member.permissions.has(discord.PermissionFlagsBits.Administrator) const discord = require('discord.js');
const audit = require('../utils/auditLog');
const db = require('../utils/database');
const formatter = require('../utils/textFormatter.js');
...
module.exports = {
  ...
  async execute(message) {
    try {
      ...
      // 🛑 STRICT ENFORCEMENT CHECK: Terminate immediately if leveling state is off or disabled
      ...
      if (userProfile.xp >= xpNeeded) {
        ...
        // 📢 ROUTE MESSAGE: Route message to config target channel or fall back to the chat source
